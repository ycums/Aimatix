...
#### 2.2.1 時刻入力
- **初期値**:
  - 絶対入力(ABS_TIME_INPUT): `__:_0`
  - 相対入力(REL_PLUS_TIME_INPUT): `__:__`
- **入力形式**: HH:MM（4桁）
- **入力範囲**: 00:00-23:59
- **入力方法**: 桁ごと編集
- **バリデーション**: 時刻範囲チェック
...
- **一括操作**: 全削除機能
- **ナビゲーション**: 端で停止（循環なし）
- **削除後選択位置**: 削除されたアラームがあった位置（最後の場合は前の位置）
- **空リスト表示**: "NO ALARMS"（中央表示）
- **選択表示**: アンバー背景、黒文字（反転）
...
### 2.6 時刻同期（起動時）
- **無効判定**: `TimeValidationLogic` により、基準日 `2025-01-01 00:00` 未満は「無効」。
- **既定ポリシー**: 無効時は起動直後に自動で時刻同期を開始（提案ダイアログなし）。
- **同期手段**: SoftAP + QR によるローカル時刻反映のみ。
- **画面遷移**: `BOOT` → `TIME SYNC` → 成功: `MAIN_DISPLAY` ／ キャンセル・タイムアウト: `SETTINGS_MENU`。
- **将来の設定化**: 将来的に「自動/提案/なし」を設定で切替可能とする余地はあるが、現時点では未提供。
 - **入力バリデーション**:
 ...
 - **エラー時の動作**:
   - `/time/set` は `400 Bad Request` を返し、`reason` に `"time_out_of_range" | "tz_offset_out_of_range"` を設定
   - システム時刻は変更せず、UIに警告を表示（警告内容は `TIME SYNC` 画面仕様に準拠）
...
### 3.2 品質要件
- **Unit Testカバレッジ**: 純粋ロジック85%以上
- **統合テストカバレッジ**: 全体（UI/アダプタ含む）80%以上
- **ビルド成功率**: 100%
- **残り時間表示精度**: ±1秒以内
- **プログレスバー**: バーは右端から左端へ向かって縮小し、長さは「残り時間 ÷ 開始時点の残り時間」の割合で決まる。精度は±1%以内。
- **アラーム鳴動精度**: ±0.1秒以内
...
- [ ] 全仕様の100%実装
- [ ] 残り時間表示精度±1秒以内
- [ ] プログレスバーの長さが「残り時間 ÷ 開始時点の残り時間」の割合で正しく変化し、精度±1%以内
- [ ] アラーム鳴動精度±0.1秒以内
- [ ] DIパターンの正しい活用確認

### 5.2 品質指標
- [ ] Unit Testカバレッジ85%以上（純粋ロジック）
- [ ] 統合テストカバレッジ80%以上（全体）
- [ ] ビルド成功率100%
- [ ] メモリ使用量最適化達成
- [ ] 純粋ロジックテストの100%成功
