...
- [ ] 他のモジュールのテスト環境整備（Phase 0.9で対応）【要再検証】
...
- ❌ 他のモジュールのテスト環境整備が必要【要再検証】
- ❌ 全主要機能のテストカバーは未達成【要再検証】
- ⚠️ Phase 1開発の品質保証基盤は部分的に確立【要再検証】
...
**統一項目**:
1. **Button型の完全統一**
   - [ ] 全テストファイルでmock_m5stack.hのButton型を使用【要再検証】
   - [ ] 状態遷移システムのButton型依存を修正【要再検証】
   - [ ] 独自MockButtonクラスの削除【要再検証】

2. **テストファイルの修正**
   - [ ] `test/test_button_manager_pure.cpp`のButton型統一【要再検証】
   - [ ] `test/test_debounce_manager_pure.cpp`のButton型統一【要再検証】
   - [ ] その他のテストファイルのButton型統一【要再検証】

3. **状態遷移システムのテスト対応**
   - [ ] 状態遷移システムの純粋ロジックテスト化【要再検証】
   - [ ] ButtonEvent、SystemState等のテスト対象化【要再検証】
   - [ ] 統合テストの拡張【要再検証】

4. **ビルド環境の確認**
   - [ ] TESTINGマクロによる環境切り替えの動作確認【要再検証】
   - [ ] 全テストのビルド・実行成功確認【要再検証】
   - [ ] カバレッジ測定の動作確認【要再検証】
...
**前提条件**: Phase 1の基盤技術整備が完了していること

**UI/UX改善の優先順位**:

**2.1 高優先度（Phase 2.1-2.3で実装）**

**2.1.1 メイン表示画面の詳細作り込み**
...
#### 2.1 メイン表示画面の詳細作り込み
**目標**: 残り時間計算、進捗バー、アラームリスト表示、DIパターン対応
**テスト項目**:
- [ ] 残り時間計算テスト（正負値、日付跨ぎ）
- [ ] 進捗率計算テスト（境界値、異常値）
- [ ] アラームリスト表示テスト（最大5件制御）
...
**実装ファイル**:
- `src/alarm.cpp`: 大幅修正（アラーム管理機能の完成）
- `src/ui.cpp`: 大幅修正（メイン表示画面の完成、進捗バー実装）
...
#### 2.2 入力モードの詳細ロジック
**目標**: 桁ごと編集、プレビュー表示、バリデーション、DIパターン対応
**テスト項目**:
- [ ] 桁ごと編集テスト（4桁、カーソル移動）
...
**実装ファイル**:
- `src/input.cpp`: 大幅修正（バリデーション強化、エラー処理改善）
- `src/ui.cpp`: 入力UI部分修正
...
#### 2.3 アラーム鳴動状態の詳細
**目標**: フラッシュ点滅、オーバータイム表示、音制御、DIパターン対応
**テスト項目**:
- [ ] フラッシュ点滅テスト（0.5秒間隔）
...
**目標**: 設定変更、確認ダイアログ、INFO画面、DIパターン対応
**テスト項目**:
- [ ] 設定項目選択・変更テスト
...
#### 2.5 電源管理機能の基本実装
**目標**: 基本的な電源管理機能の実装、DIパターン対応
**前提条件**:
- Phase 0.9.5のButton型完全統一が完了していること
...
**実装ファイル**:
- `src/power_manager.cpp`: 新規作成（電源管理機能）
- `src/power_manager.h`: 新規作成（電源管理機能）
- `src/main.cpp`: loop関数の修正（ループ処理の最適化、電源管理統合）
