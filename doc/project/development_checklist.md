...
- [x] 3-0-10: 相対値入力ができる
  ...
  - [x] アラームが設定できる
  - [x] 静的解析実行（Clang-Tidy）
- [x] 3-0-11: アラーム管理画面（AlarmDisplay）雛形追加・遷移（AlarmLogic連携、リスト表示・選択）
  ...
  - 詳細な品質ゲート基準については [品質ゲート基準](../operation/quality_gates.md) を参照してください。
  - [x] 静的解析実行（Clang-Tidy） ← 一部未達だがOKとした。
- [x] 3-0-11.5: 相対入力モードのバグ修正
  - [x] 相対時間→絶対時間の変換時に秒以下が反映されていない。
    - [x] now: 15:34:12 & input: __:05 なら expected: 15:39:12
      - [x] actual: 15:39:00
- [x] 3-0-11.6: アラームリストの行高さの修正
- [x] 3-0-12: 設定画面（SettingsDisplay）雛形追加・遷移（SettingsLogic stub、設定値の表示・編集雛形）
  - [x] 静的解析実行（Clang-Tidy）
- [x] 3-0-13: 日時手動設定画面の追加（EEPROM非依存）
  - [x] 設定画面に「Set Date/Time」項目を追加
  - [x] 日付・時刻入力画面の実装（YYYY/MM/DD HH:MM形式）
  - [x] 入力値のバリデーション（日付範囲、時刻範囲）
  - [x] 確定時にシステム時刻を更新（揮発性、電源OFFでリセット）
  - [x] 静的解析実行（Clang-Tidy）
- [x] 3-0-13.1: 日時入力機能の修正
  - [x] カーソル表示位置の修正（最優先）
  - [x] 入力範囲制限の修正（年千の位、年百の位を入力不可に）
  - [x] カーソル移動の循環化
  - [x] システム時刻更新の実装
  - [x] 表示品質の改善（ちらつき解消）
- [x] 3-0-14:  bug: 絶対時刻入力、相対時刻入力で1桁目が未入力のときの設定値が間違っている
- [x] 3-0-15: アラーム一覧が2度目以降正しく表示されないバグ
  - [x] 実際には アラームリストは空ではないがアラーム一覧に0項目表示される
    - [x] "NO ARARMS" とも表示されずただ空欄になっている。
  - [x] 初回表示時には正しく表示される
