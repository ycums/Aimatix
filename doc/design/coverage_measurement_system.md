...
詳細な品質ゲート基準の数値は`coverage_config.json`を正典とし、方針は [品質ゲート基準](../operation/quality_gates.md) を参照してください。
...
#### 4.1.2 品質ゲート
- 具体的な閾値は`coverage_config.json`の`quality_gate`を参照（本ドキュメントでは方針のみ記載）。
- 通知・連携は必要に応じて運用で追加。
