...
bool AlarmLogic::addAlarmFromPartialInput(
    ...
    if (entered[0] && entered[1]) {
        // 両方入力済み
        hour = digits[0] * SECONDS_10 + digits[1];
    } else if (entered[0] && !entered[1]) {
        // 時十桁のみ入力済み → 時一桁を0として補完
        hour = digits[0] * SECONDS_10 + 0;
    } else if (!entered[0] && entered[1]) {
        // 時一桁のみ入力済み → 時十桁を0として補完
        hour = 0 * SECONDS_10 + digits[1];
    } else {
        // 時が未入力
        hour = 0;
    }
    ...
    if (entered[2] && entered[3]) {
        // 両方入力済み
        minute = digits[2] * SECONDS_10 + digits[3];
    } else if (entered[2] && !entered[3]) {
        // 分十桁のみ入力済み → 分一桁を0として補完
        minute = digits[2] * SECONDS_10 + 0;
    } else if (!entered[2] && entered[3]) {
        // 分一桁のみ入力済み → 分十桁を0として補完
        minute = 0 * SECONDS_10 + digits[3];
    } else {
        // 分が未入力
        minute = 0;
    }
