...
void drawButtonHintsGrid(IDisplay* disp, const char* btnA, const char* btnB, const char* btnC) {
    ...
    disp->setTextColor(AMBER_COLOR, TFT_BLACK);
    disp->setTextDatum(TL_DATUM);
    if (btnA) disp->drawText(BUTTON_A_OFFSET_X, SCREEN_HEIGHT - HINT_HEIGHT + HINT_OFFSET_Y, btnA, FONT_AUXILIARY);
    if (btnB) disp->drawText(SCREEN_WIDTH / 2 - BUTTON_B_OFFSET_X, SCREEN_HEIGHT - HINT_HEIGHT + HINT_OFFSET_Y, btnB, FONT_AUXILIARY);
    if (btnC) disp->drawText(SCREEN_WIDTH - BUTTON_C_OFFSET_X, SCREEN_HEIGHT - HINT_HEIGHT + HINT_OFFSET_Y, btnC, FONT_AUXILIARY);
}
